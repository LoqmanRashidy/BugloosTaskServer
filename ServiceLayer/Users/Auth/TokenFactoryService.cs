...
namespace ServiceLayer.Users
{
    ...
    public class TokenFactoryService : ITokenFactoryService
    {
      ...
      ILogger<TokenFactoryService> logger)
        ...
        public async Task<(string AccessToken, IEnumerable<Claim> claims)> createAccessTokenAsync(User user)
        {
            ...
            var claims = new List<Claim>
            {
                ...
                new Claim("Name", user.Name,ClaimValueTypes.String, _configuration.Value.Issuer),
            ...
            {
                claims.Add(new Claim(ClaimTypes.Role, role.EnTitle, ClaimValueTypes.String, _configuration.Value.Issuer));
            }
            var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_configuration.Value.Key));
            var creds = new SigningCredentials(key, SecurityAlgorithms.HmacSha256);
            var now = DateTime.Now;
            int accessTokenExpiration = _configuration.Value.AccessTokenExpiration;
             ...
             token = new JwtSecurityToken(
              ...
              expires: now.AddHours(accessTokenExpiration),
        ...
        private (string RefreshTokenValue, string RefreshTokenSerial) createRefreshToken()
        {
            ...
            int refreshTokenExpiration = _configuration.Value.RefreshTokenExpiration;
            token = new JwtSecurityToken(
                issuer: _configuration.Value.Issuer,
                audience: _configuration.Value.Audience,
                claims: claims,
                notBefore: now,
                expires: now.AddHours(refreshTokenExpiration),
                signingCredentials: creds);
